...
- **Library**: `GET /api/audio-library` lists, `GET /api/audio-library/:name` serves, files under `AUDIO_LIBRARY_DIR` (default `/var/lib/sonicroom/media`). `isAudioFileName` enforces a basename-only, no-dotfile, audio-extension allowlist; `sendFile` is rooted with `dotfiles: deny` — no path traversal.
